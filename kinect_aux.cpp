...
//		ros::shutdown();
		fprintf( stderr, "Error in accelerometer reading, libusb_control_transfer returned %d", ret );
		exit( EXIT_FAILURE );
	}
	...
	*/
	printf( "% 12f, %u\n", double(tilt_angle) / 2., tilt_status );
}
...
//		ros::shutdown();
		fprintf( stderr, "Error in setting tilt angle, libusb_control_transfer returned %d\n", ret );
		exit( EXIT_FAILURE );
	}
}
...
//		ros::shutdown();
		fprintf( stderr, "Error in setting LED options, libusb_control_transfer returned %d\n", ret );
		exit( EXIT_FAILURE );
	}
}
